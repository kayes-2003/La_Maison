      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-sm flex flex-col bg-surface-50 border-l border-brand-800/30 shadow-2xl animate-slide-in">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {summary.items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-brand-800 py-12">
              ...
              {onViewOrders && (
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-brand-700/40 text-brand-400 text-sm font-semibold hover:bg-brand-900/40 hover:border-brand-600/50 transition-all"
                >
            ...
            summary.items.map(ci => {
              ...
              return (
                <div
                  key={ci.id}
                  className="flex gap-3 bg-surface-100/60 border border-brand-900/30 rounded-xl p-3 transition-all hover:border-brand-800/50"
                >
                  {/* Thumbnail */}
