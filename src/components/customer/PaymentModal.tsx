...
export function PaymentModal({ summary, onClose, onSuccess }: PaymentModalProps) {
  ...
  const initPayment = async () => {
    ...
    const isConfigured =
      supabaseUrl && !supabaseUrl.includes('YOUR_PROJECT') && stripePromise
  ...
  const inputCls =
    'w-full bg-surface-100 border border-brand-800/50 rounded-lg px-3.5 py-2.5 text-brand-100 text-sm placeholder-brand-800 outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500/30 transition-all'
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative w-full sm:max-w-md bg-surface-50 border border-brand-900/40 sm:rounded-2xl shadow-2xl animate-fade-up overflow-hidden max-h-[95vh] flex flex-col rounded-t-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-brand-900/30 shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-brand-700 hover:text-brand-300 hover:bg-brand-900/40 transition-colors"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-5">
          ...
          <div className="bg-surface-100/60 border border-brand-900/30 rounded-xl p-4 mb-5">
            <div className="space-y-1.5 mb-3">
              {summary.items.slice(0, 4).map(ci => (
                <div key={ci.id} className="flex justify-between text-sm">
                  <span className="text-brand-500 truncate max-w-[180px]">
                    {ci.quantity}× {ci.menu_items.name}
                  </span>
                  <span className="text-brand-400 font-mono text-xs">
                    {formatPrice(ci.menu_items.price * ci.quantity)}
                  </span>
                </div>
              ))}
              {summary.items.length > 4 && (
                <p className="text-brand-700 text-xs">+{summary.items.length - 4} more items</p>
              )}
            </div>
            <div className="flex justify-between items-center pt-2.5 border-t border-brand-900/30">
              ...
              <span className="font-display font-bold text-brand-300 text-xl">
            ...
            <Elements
              ...
              options={{
                ...
                appearance: {
                  ...
                  rules: {
                    '.Input': { border: '1px solid rgba(99,51,21,0.5)', backgroundColor: '#221608' },
                    '.Input:focus': { border: '1px solid #d4841a', boxShadow: '0 0 0 3px rgba(212,132,26,0.15)' },
                  },
                },
              }}
            >
              <CheckoutForm
                ...
                onError={msg => { setErrorMsg(msg); setModalState('error') }}
              />
            </Elements>
          )}
          ...
          {modalState === 'demo' && (
            <form onSubmit={handleDemoSubmit} className="space-y-4">
              <div className="flex items-center gap-2 px-3 py-2 bg-amber-950/40 border border-amber-800/40 rounded-lg">
              ...
              <div>
                ...
                <input
                  ...
                  value={demoCard}
                  onChange={e => setDemoCard(formatCard(e.target.value))}
                  className={inputCls}
                  maxLength={19}
                  required
                />
              ...
              <p className="text-center text-brand-800 text-xs">
          ...
          {modalState === 'error' && (
            <div className="text-center py-8 space-y-4">
              ...
              <button onClick={initPayment} className="btn-primary w-full py-3">
                Try again
              </button>
            </div>
          )}
