...
export function Header({
  profile, cartCount, onSignIn, onCartOpen, onSignOut, onAdminPanel, onNavigate, activeView,
}: HeaderProps) {
            ...
            {NAV_LINKS.map(({ label, view }) => (
              <button
                ...
                className={cn(
                  ...
                  activeView === view
                    ? 'text-brand-200 bg-brand-900/50'
                    : 'text-brand-600 hover:text-brand-300 hover:bg-brand-900/30',
                )}
              >
                {label}
              </button>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-brand-900/30 bg-surface-50 animate-fade-up">
          <div className="px-4 py-3 flex flex-col gap-2">
            {/* Nav links */}
            <div className="flex flex-col gap-1 pb-2 border-b border-brand-900/20 mb-1">
              {NAV_LINKS.map(({ label, view }) => (
                <button
                  key={view}
