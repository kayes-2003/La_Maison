...
import { Plus, LayoutGrid, List, BarChart2, Receipt, UtensilsCrossed, Image, Megaphone } from 'lucide-react'
...
type Tab = 'dashboard' | 'orders' | 'menu' | 'banners'
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-brand-700 text-sm mt-0.5">Manage the full restaurant experience</p>
        </div>
        {tab === 'menu' && (
      ...
      {/* Tab bar */}
      <div className="flex gap-1 bg-surface-100/60 border border-brand-900/30 rounded-xl p-1 mb-6 w-fit flex-wrap">
        {tabs.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
      ...
      {/* Orders */}
      {tab === 'orders' && (
        dash.loading ? <Loader /> : <OrdersTab orders={dash.orders} onRefresh={dash.reload} />
      )}
